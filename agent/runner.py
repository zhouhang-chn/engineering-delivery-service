...
def main(argv: list[str] | None = None) -> int:
    ...
    update_work_order(
        work_order_id,
        original_request=args.requirement,
        confirmed_requirement=args.requirement,
        repository=args.repo_url or _resolve_repo_url(base_dir),
        overall_status="in_progress",
    )
    configure_supervisor(
        worker=ScriptedWorker(args.scripted) if args.scripted else CodexWorkerDriver(),
        base_dir=base_dir,
        port=args.port,
    )
    _log(f"work order {work_order_id}: supervising '{args.requirement}'")
    result = supervise(work_order_id, turn_budget=args.turn_budget)

    print()
    print(f"work order   : {result.work_order_id}")
    print(f"turns        : {result.turns}/{result.turn_budget}")
