...
| **v0.1.4-a2a-endpoint** | A2A endpoint, completion artifacts, full-chain e2e, dogfooding via a raw A2A client | A2A task in → `completed` with artifacts |
| **v0.1.5-eds-cli** | `eds` CLI as the reference external caller: serve / submit / status --watch / open; user quickstart | user tests EDS in four commands; M1 exit criteria demo |

Estimated production LOC: ~700 / ~450 / ~350 / ~350 / ~300 (total ≈ 2,150,
within the 1,000–3,000 version budget).

Dependency order is strict: 1.1 → 1.2 → 1.3 → 1.4 → 1.5. The riskiest
unknown (Codex App Server binding) is spiked in the first tasks of v0.1.1.
A web UI is deliberately deferred — the CLI is the external test surface,
and the delivered API's `/docs` remains the acceptance surface.
